...
// [FIX] 原 ExtractDomainFromUrl 会在第一个 ':' 处截断，导致端口丢失
// 新版本保留完整的 host:port 部分，只去掉 scheme 和 path
...
std::string NetworkFilterPlugin::ExtractHostPortFromUrl(const std::string& url) {
    ...
    // [FIX] 只用 '/' 作为结束符，保留 host:port 中的冒号
...
// [FIX] 从 "host:port" 字符串中分离出 host 和 port
void NetworkFilterPlugin::SplitHostPort(const std::string& hostPort,
    ...
    if (hostPort[0] == '[') {
        ...
        if (bracketEnd != std::string::npos) {
            host = hostPort.substr(1, bracketEnd - 1);  // 去掉方括号
...
bool NetworkFilterPlugin::HandleClientConnection(void* clientSocket) {
    ...
    // [FIX] CONNECT 默认 443，其他 HTTP 方法默认 80
    ...
    if (isConnect) {
        ...
        // 读取请求头, 同时解析 Content-Length 和 Transfer-Encoding
